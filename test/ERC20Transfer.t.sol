...
contract ERC20TransferTest is Test {
    ...
    bytes32 private constant TEST_NODE = keccak256(abi.encodePacked(bytes32(0), keccak256("test")));
    ...
    function setUp() public {
        ...
        // Set up test node in ENS Registry
    ...
    function test_ContractDeployment() public {
        assertEq(address(erc20Transfer.pnsRegistry()), address(pnsRegistry), "ENS Registry address should match");
    }
    
    function test_TransferToENS() public {
        ...
        // Execute transfer to ENS
        vm.deal(address(this), transferAmount);
        vm.expectEmit(true, true, true, true);
        emit TransferToENS(address(this), TEST_NODE, transferAmount);
        erc20Transfer.transferToENS{value: transferAmount}(TEST_NODE);
    ...
    function test_RevertWhenNoETHSent() public {
        ...
        erc20Transfer.transferToENS{value: 0}(TEST_NODE);
    }
    
    function test_RevertWhenNoResolverSet() public {
        // Create a new node with no resolver
        bytes32 noResolverNode = keccak256(abi.encodePacked(bytes32(0), keccak256("noresolver")));
        vm.prank(ADMIN);
        ...
        erc20Transfer.transferToENS{value: 1 ether}(noResolverNode);
    }
    
    function test_RevertWhenNoAddressSet() public {
        // Create a node with resolver but no address set
        bytes32 noAddrNode = keccak256(abi.encodePacked(bytes32(0), keccak256("noaddr")));
        ...
        erc20Transfer.transferToENS{value: 1 ether}(noAddrNode);
    }
    
    function test_TransferToMultipleAddresses() public {
        // Setup multiple ENS nodes and addresses
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            // Transfer ETH
            vm.deal(address(this), transferAmount);
            erc20Transfer.transferToENS{value: transferAmount}(nodes[i]);
    ...
    function test_TransferFailure() public {
        ...
        erc20Transfer.transferToENS{value: 1 ether}(TEST_NODE);
    }
